...
public abstract class Calendar {
	...
	public int get(int field) {
		switch (field) {
			...
			case MONTH:
				return date.getMonth(); // 0-indexed in both Calendar and Date
			case DAY_OF_MONTH:
				return date.getDate(); // 1-indexed
			case DAY_OF_WEEK:
				return date.getDay() + 1; // Calendar is 1-indexed (1 = Sunday)
			case DAY_OF_YEAR:
				return computeDayOfYear();
			default:
				return 0;
		}
	}
	...
	public int getActualMaximum(int field) {
		if (field == DAY_OF_YEAR) {
			return isLeapYear(date.getYear() + 1900) ? 366 : 365;
		}
		return 0;
	}
